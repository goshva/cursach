...
class Registr_win(QMainWindow, Ui_widget):
    def __init__(self):
        ...
        self.role_us = self.lineEdit.text()

        self.reg_btn.clicked.connect(lambda: self.reg(self.username, self.password, self.email, self.role_us))

    def reg(self, username, password, email, role_us):
        ...
        if value != []:
            ...
            cur.execute(f"INSERT INTO users (username, password, email, role) VALUES ('{username}', '{password}', '{email}', '{role_us}')")
            con.commit()
            con.close
